...
// ==========================================
// 4. MANAGE STUDENTS (Add, List, Delete, History)
// ==========================================
...
function generateQRCode(id, name, section) {
    ...
    document.getElementById('qr-label').innerText = `${name} (${section})`;
}
        ...
        querySnapshot.forEach((doc) => {
            ...
            li.innerHTML = `
                <div>
                    ...
                    <small style="color:#555;">ID: ${student.student_id}</small>
                </div>
                <div>
...
function processAttendance(studentId) {
    ...
    .then((querySnapshot) => {
        ...
        if (classStartTime) {
            ...
            if (diffMins > 45) { status = "Absent/Very Late"; colorClass = "status-absent"; }
...
function loadReports() {
    ...
    db.collection("attendance")
        ...
        .then((querySnapshot) => {
            tableBody.innerHTML = '';
            if (querySnapshot.empty) return tableBody.innerHTML = '<tr><td colspan="3">No records found for this date.</td></tr>';
...
function downloadCSV() {
    ...
    let csvContent = "data:text/csv;charset=utf-8,Time,Name,Status\n";
